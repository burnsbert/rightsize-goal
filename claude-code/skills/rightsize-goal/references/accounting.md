...
5. Once a child has delivered its receipt and gone idle, or the runtime confirms it has stopped, evaluate its evidence and invoke `finish` with the coordinator's outcome (`accepted`, `rework`, `unresolved`, `cancelled`, or `failed`) before assigning it more work. An idle teammate has finished its assignment: do not stop it just to run `finish`. Stopping an agent requires confirming it has stopped; an interrupt request alone does not prove that final usage has been written. Repeated finish calls are idempotent and must not count the task twice. The first `finish` also reports the worker's context facts: `context_tokens` (its context at the latest request), `context_window`, `context_free_pct`, `eligible` (at least 30% free and not compacted), `compacted`, `assignment_growth_tokens` (how much this assignment alone added), `oversized` (that growth reached 30% of the window), and a `context_advice` line. Apply the skill's worker lifecycle rules to them.
...
python3 <skill-dir>/scripts/task_usage.py workers --run <run-id>
python3 <skill-dir>/scripts/task_usage.py stop --run <run-id> --agent-id <exact-agent-id> --reason "idle 15 minutes"
```

`workers` derives each worker's status (working, idle, or stopped), idle minutes, context facts, and `gc_due` from its last line in the goal log. Run `stop` after stopping a worker with `TaskStop`, so the log no longer counts it as idle.

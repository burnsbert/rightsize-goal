...
If none of the four roles is available, say so plainly and stop rather than substituting a default agent: the roles exist to pin model and effort, and the Agent tool has no dispatch-time effort override.
...
- **Effort.** Every tier except the Haiku junior pins an effort level, and effort drives how many tokens a model spends at a fixed rate. Claude Code's session default is `xhigh`; every effort-bearing role here runs at the documented API default of `high`, not above it.
...
| `rightsize-senior-doer` (senior engineer) | Opus 5 / high | Hard work beyond bounded midlevel scope: deeply ambiguous debugging, unfamiliar complex integrations, substantive research, consequential design tradeoffs, difficult new hypotheses, or deep interacting constraints. No cheaper failure is required first. |
| `rightsize-principal-doer` (principal engineer / lead architect) | Fable 5.1 / high | The hardest bounded unresolved implementation, reasoning, or fresh-direction consultation after documented Opus struggle. |

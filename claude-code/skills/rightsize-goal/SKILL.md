...
When the main session is Opus, follow the lightweight-coordinator boundary below. Choosing Opus for orchestration does not make it the default executor.
...
- The intended top-level session is Opus at `medium` effort, acting as the manager. A skill's frontmatter can only override model and effort for one turn, so this skill does not attempt it. Ask the user to run `/model opus` and `/effort medium` for the session. Read the actual coordinator effort from the `CLAUDE_EFFORT` environment variable and the actual coordinator model from the session transcript; record both, or `unknown`. Disclose a mismatch once and continue under the manager boundary without changing global defaults or falsely labeling the model.
...
| --- | --- | --- |
| Main session (manager) | Opus / medium | Own priorities, goal state, routing, acceptance decisions, and escalation. Delegate engineering execution to the cheapest capable doer; keep tiny coordination and checks local when dispatch costs more. |
...
- Give each spawned agent a stable `name` so it can be addressed later, and record the agent ID the tool returns. Reuse an existing suitable agent for related follow-ups with `SendMessage`; give each new assignment a new task ID and record the reused agent ID. Do not send cheaper work to an existing expensive agent just because it is idle. Parallelize only independent assignments with distinct ownership and useful coordinator work; send parallel dispatches in a single message.
- The main session alone owns the goal, the scratch log, and the gate state. Integrate completed results, verify appropriate acceptance evidence, and record accepted, rework, unresolved, or cancelled outcomes. An agent saying "done" is not completion evidence. Verification that itself stretches a cheap tier goes to Opus; avoid duplicating an entire investigation without cause.

...
- **Effort.** Every tier except the Haiku junior pins an effort level, and effort drives how many tokens a model spends at a fixed rate. Claude Code's session default is `xhigh` and the API default is `high`; every effort-bearing role here deliberately runs at or below `high`. Choosing `rightsize-lower-senior-doer` over `rightsize-senior-doer` is a real cost decision even though both are Opus.
...
| `rightsize-upper-midlevel-doer` (upper-midlevel engineer) | Sonnet 5 / high | Moderately complex bounded work needing more judgment: multi-file changes in established architecture, localized debugging across known components, focused research synthesis, and practical design options within existing constraints. |
...
| `rightsize-senior-doer` (senior engineer) | Opus 5 / high | Deep interacting constraints or difficult reasoning where more effort on the same model is likely to pay off; may receive such tasks directly or after Opus/medium stalls. |
| `rightsize-staff-doer` (staff engineer) | Fable 5.1 / medium | Bounded expert implementation, reframing, architecture review, research synthesis, or a discriminating next step after documented Opus struggle. Complexity, prestige, or wanting extra reassurance alone does not qualify. |
| `rightsize-principal-doer` (principal engineer / lead architect) | Fable 5.1 / high | The hardest bounded unresolved implementation, reasoning, or fresh-direction consultation after documented Opus struggle, when the staff tier's lower effort is unlikely to be sufficient. |

...
If deeper analysis yields no defensible next step, return the attempted approaches and their evidence, the unresolved question, and what a principal-level consultation should resolve. Do not escalate yourself, spawn agents, change the session goal, or write the coordinator's scratch log or gate state. Return when the deliverable or stop condition is reached.
